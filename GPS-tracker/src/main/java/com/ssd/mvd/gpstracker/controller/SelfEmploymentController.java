...
import com.ssd.mvd.gpstracker.database.SerDes;
import com.ssd.mvd.gpstracker.request.SelfEmploymentRequest;
import com.ssd.mvd.gpstracker.response.ApiResponseModel;
import com.ssd.mvd.gpstracker.response.Status;
import com.ssd.mvd.gpstracker.task.card.ReportForCard;
...
//    public Mono<> getDetails ( Patrul patrul ) {}
    ...
    public Mono< ApiResponseModel > addReportForSelfEmployment ( ReportForCard reportForCard ) { return reportForCard.getCardType().equals( "selfEmployment" ) ? Archive.getAchieve().get( reportForCard.getCardId() )
            .flatMap( selfEmploymentTask -> {
                ...
                return RedisDataControl.getRedis().getPatrul( reportForCard.getPassportSeries() ).flatMap( patrul -> Mono.just( ApiResponseModel.builder().success( CassandraDataControl.getInstance().addValue( selfEmploymentTask, SerDes.getSerDes().serialize( selfEmploymentTask ) ) ).success( CassandraDataControl.getInstance().addValue( patrul.changeTaskStatus( com.ssd.mvd.gpstracker.constants.Status.FINISHED ), SerDes.getSerDes().serialize( patrul ) ) ).status( Status.builder().code( 200 ).message( patrul.getName() + " report's was saved to selfEmploymentTask" ).build() ).build() ) );
