...
import com.ssd.mvd.gpstracker.task.selfEmploymentTask.SelfEmploymentTask;
import lombok.Data;
...
public class Archive implements Runnable {
    ...
    private final Map< com.ssd.mvd.gpstracker.constants.Status, List< Patrul > > patrulMonitoring = new HashMap<>(); // to check all Patruls

    public static Archive getAchieve () { return archive != null ? archive : ( archive = new Archive() ); }

    private Archive() {
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.NOT_AVAILABLE, new ArrayList<>() );
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.AVAILABLE, new ArrayList<>() );
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.ATTACHED, new ArrayList<>() );
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.FINISHED, new ArrayList<>() );
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.ARRIVED, new ArrayList<>() );
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.BUSY, new ArrayList<>() );
        this.getPatrulMonitoring().put( com.ssd.mvd.gpstracker.constants.Status.FREE, new ArrayList<>() );
    ...
    public Mono< ApiResponseModel > save ( UUID uuid, Patrul patrul ) { return this.get( uuid ).flatMap( selfEmploymentTask -> {
        selfEmploymentTask.getPatruls().add( patrul.changeTaskStatus( com.ssd.mvd.gpstracker.constants.Status.ATTACHED ).getPassportNumber() );
    ...
    public Mono< ApiResponseModel > save ( Patrul patrul, UUID cardId ) { return this.getCard( cardId ).flatMap( card -> {
        ...
        card.getPatruls().add( patrul.changeTaskStatus( com.ssd.mvd.gpstracker.constants.Status.ATTACHED ) );
    ...
    public Mono< ApiResponseModel > save ( SelfEmploymentTask selfEmploymentTask, Patrul patrul ) {
        if ( !this.selfEmploymentTaskMap.containsKey( selfEmploymentTask.getUuid() ) ) {
            ...
            patrul.setStatus( com.ssd.mvd.gpstracker.constants.Status.BUSY );
            ...
            CassandraDataControl.getInstance().addValue( selfEmploymentTask, SerDes.getSerDes().serialize( selfEmploymentTask ) );
            patrul.changeTaskStatus( com.ssd.mvd.gpstracker.constants.Status.ARRIVED ).setSelfEmploymentId( selfEmploymentTask.getUuid() );
//            this.save( Notification.builder().patrul( patrul ).status( false ).title( patrul.getName() + " set the Task to himself" ).notificationWasCreated( new Date() ).build() );
    ...
    public Mono< ApiResponseModel > removePatrulFromSelfEmployment ( UUID uuid, Patrul patrul ) { return this.selfEmploymentTaskMap.containsKey( uuid ) ?
            this.get( uuid ).flatMap( selfEmploymentTask -> {
                selfEmploymentTask.getPatruls().remove( patrul.changeTaskStatus( com.ssd.mvd.gpstracker.constants.Status.FINISHED ) );
                CassandraDataControl.getInstance().addValue( selfEmploymentTask, SerDes.getSerDes().serialize( selfEmploymentTask ) );
    ...
    public void run () {
        while ( this.getFlag() ) { RedisDataControl.getRedis().getAllPatruls().subscribe( patrul -> {
            ...
            if ( !patrul.getStatus().equals( com.ssd.mvd.gpstracker.constants.Status.NOT_AVAILABLE ) ) {
                ...
                if ( !patrul.getStatus().equals( com.ssd.mvd.gpstracker.constants.Status.FREE ) ) this.getPatrulMonitoring().get( patrul.getTaskStatus() ).add( patrul );
