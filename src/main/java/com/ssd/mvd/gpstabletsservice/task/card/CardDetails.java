...
//                        case  "ПОДРАЗДЕЛЕНИЕ" -> this.getDetails().get( Details.DETAILS ).add( new Item( s, card.getPatruls().get(0).getPoliceType() ) );
    ...
    public CardDetails ( EscortTuple escortTuple, String ru, TupleOfCar tupleOfCar ) {
        ...
        this.getDetails().get( Details.ESCORT ).add( new Item( "ID", escortTuple.getUuid() ) );
        this.getDetails().get( Details.ESCORT ).add( new Item( "Страна", escortTuple.getCountries() ) );
        this.getDetails().get( Details.ESCORT ).add( new Item( "Полигон", escortTuple.getUuidOfPolygon() ) );

        this.getDetails().get( Details.ESCORT_CAR ).add( new Item( "Гос. номер машины", tupleOfCar.getGosNumber() ) );
        this.getDetails().get( Details.ESCORT_CAR ).add( new Item( "Тип порученной машины", tupleOfCar.getCarModel() ) );
        this.getDetails().get( Details.ESCORT_CAR ).add( new Item( "Широта локации машины", tupleOfCar.getLatitude() ) );
        this.getDetails().get( Details.ESCORT_CAR ).add( new Item( "Долгота локации машины", tupleOfCar.getLongitude() ) ); }
